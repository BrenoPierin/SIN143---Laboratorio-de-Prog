...
export async function deletarProduto(id) {
  const resp = await apiFetch(`/produtos/${id}/`, { method: 'DELETE' });
  if (!resp.ok) throw new Error('Erro ao deletar produto.');
}

export async function uploadImagemProduto(produtoId, formData) {
  const token = getAccessToken();
  const resp = await fetch(
    `https://dashier-brynn-electrotechnic.ngrok-free.dev/api/produtos/${produtoId}/imagens/`,
    {
      method: 'POST',
